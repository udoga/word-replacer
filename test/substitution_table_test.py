...
class SubstitutionTableTest(TestCase):
    ...
    def test_averages_values_in_frames(self):
        ...
        for df in [df1, df2, df3]: df.index.name = 'token'
        sum_frame = SubstitutionTable.sum_frames_by_token([df1, df2])
        avg_frame = SubstitutionTable.avg_frames_by_token([df1, df2])
